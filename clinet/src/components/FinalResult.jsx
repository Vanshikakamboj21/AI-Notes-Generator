    ...
    <div className="mt-6 p-3 space-y-10 bg-white">
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        ...
        <div className="flex gap-3">
          ...
          <button
          onClick={()=>downloadPdf(result)} className="px-4 py-2 rounded-lg text-sm font-medium bg-indigo-600 text-white hover:bg-indigo-700 transition">
            ⬇️ Download PDF
          </button>
        </div>
      </div>
      ...
      {!quickRevision && (
        <section>
          {/* FIXED HERE: icon instead of icons */}
...
<section>
  ...
  <ul className="list-disc ml-6 text-gray-700 space-y-1">
   <li>{result.questions.diagram || "None"}</li>
  </ul>
</section>
    </div>
  );
